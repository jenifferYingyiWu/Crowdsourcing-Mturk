function [ output_args ] = studyBatchSize(doOneStep, doIterative, doIterativeIID, rankersIcare, datasetsIcare, isAbsolute, resoRange)
import java.util.*;
overallTime = tic;
...
for dsiter=1:length(datasets)
    ...
    if isAbsolute
        reso.absolute_steps = true;
    else
        reso.absolute_steps = false;
    end
    
    for resoIdx=1:length(resoRange)
        currentBatchSize = resoRange(resoIdx);
        reso.steps = currentBatchSize;
        ...
        if doOneStep
            ...
            while rankiter.hasNext()
                ...
                outFilename = ['1-' rankerAcr '-' num2str(currentBatchSize)];
        ...
        if doIterative
            ...
            while rankiter.hasNext()
                ...
                rankerName = ranker_functions.get(rankerAcr);
                rankerFunc = str2func(ranker_functions.get(rankerAcr));
                outFilename = ['iter-' rankerAcr '-' num2str(currentBatchSize)];
        ...
        if doIterativeIID
            ...
            while rankiter.hasNext()
                ...
                rankerName = ranker_functions.get(rankerAcr);
                rankerFunc = str2func(ranker_functions.get(rankerAcr));
                outFilename = ['iid-' rankerAcr '-' num2str(currentBatchSize)];

...
{
	mexPrintf(
	"Usage: new_model = inc_lltrain(old_model, training_label_vector, training_instance_matrix, 'liblinear_options', 'col');\n"
	"liblinear_options:\n"
...
int read_problem_sparse(const mxArray *label_vec, const mxArray *instance_mat)
{
	...
	mxArray *instance_mat_col; // in3tance sparse matrix in column format
		...
		int nrhs, const mxArray *prhs[] )
{
	...
	if(nrhs > 2 && nrhs < 6)
	{
		...
		if(!mxIsStruct(prhs[0])) {
			mexPrintf("Error: old_model must be a struct\n");
			fake_answer(plhs);
                        return;
		}
                model_ = Malloc(struct model, 1);
                error_msg = matlab_matrix_to_model(model_, prhs[0]);
                if(error_msg)
                {
		...
		if(mxIsSparse(prhs[2]))
			err = read_problem_sparse(prhs[1], prhs[2]);
		else
		{
		...
		else
		{
			...
			model_ = inc_train(model_, &prob, &param);
			if (model_ != NULL) {
				...
				mexPrintf("Error: the returned model is null!");
		}
		destroy_param(&param);
		free(prob.y);
		free(prob.x);
